...
import { useCustomer, useUpdateCustomer } from '../../../src/hooks/useCustomers';
...
export default function EditCustomerScreen() {
  ...
  useEffect(() => {
    if (customer) {
      ...
      setTaxExemptNumber(customer.tax_exempt_number ?? '');
      setPaymentTerms(customer.payment_terms);
      setNotes(customer.notes ?? '');
    }
  }, [customer]);

  const canSave = hasSupabaseEnv && id && contactName.trim();
  ...
  if (!hasSupabaseEnv || !id) {
    return (
      <View style={{ flex: 1, backgroundColor: '#0F172A', padding: 24 }}>
        ...
        <Text style={{ color: '#94A3B8' }}>Connect Supabase to edit customers.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0F172A' }}>
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <TouchableOpacity onPress={handleSave} disabled={!canSave || updateCustomer.isPending} style={{ backgroundColor: canSave ? '#3B82F6' : '#334155', borderRadius: 12, padding: 16, alignItems: 'center' }}>
          {updateCustomer.isPending ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: '600', fontSize: 16 }}>Save</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
