...
import { useVendor, useUpdateVendor } from '../../../src/hooks/useVendors';
...
export default function EditVendorScreen() {
  ...
  useEffect(() => {
    if (vendor) {
      ...
      setW9Requested(vendor.w9_requested ?? false);
      setW9Received(vendor.w9_received ?? false);
      setW9ReceivedDate(vendor.w9_received_date ?? format(new Date(), 'yyyy-MM-dd'));
      setW9FileUrl(vendor.w9_file_url);
      setNotes(vendor.notes ?? '');
    }
  }, [vendor]);

  const canSave = hasSupabaseEnv && id && companyName.trim();
  ...
  if (!hasSupabaseEnv || !id) {
    return (
      <View style={{ flex: 1, backgroundColor: '#0F172A', padding: 24 }}>
        ...
        <Text style={{ color: '#94A3B8' }}>Connect Supabase to edit vendors.</Text>
      </View>
    );
  }
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        {w9Received && (
          <>
            ...
            {w9FileUrl && (
              <TouchableOpacity onPress={() => Linking.openURL(w9FileUrl!)} style={{ marginBottom: 8 }}>
                <Text style={{ color: '#3B82F6' }}>View W-9</Text>
              </TouchableOpacity>
            )}
        ...
        <TouchableOpacity onPress={handleSave} disabled={!canSave || updateVendor.isPending} style={{ backgroundColor: canSave ? '#3B82F6' : '#334155', borderRadius: 12, padding: 16, alignItems: 'center' }}>
          {updateVendor.isPending ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: '600', fontSize: 16 }}>Save</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
